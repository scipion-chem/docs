...
1) **Import**: the initial receptor and ligand structures can be imported in several ways, as we explained in the previous workflow. 
...
The forms provided by Scipion (in Fig. \ref{fig:formImport}) allow the user to choose the origin of the structure and, 
in the case of the small molecules, the molecule handler (RDKit or OpenBabel) to use and if a 3D reconstruction is needed.

2) **Preparation**: once the structures are imported into the Scipion workflow, separate preparation steps are performed 
...
(shown in Fig. \ref{fig:formPrep}), the user is asked about the preparation parameters desired, such as to removing undesired atoms 
(waters and other non-protein entities) or reconstructing missing atoms in the receptor; or which force fields to use and whether 
to generate conformers in the parametrization of the ligands.

3) **ROI definition**: in this particular example, P2Rank is used to predict the most promising pockets in our receptor, 
...
a filter protocol to extract only the 2 best pockets predicted, in order to speed up the downstream workflow. In Fig. \ref{fig:formROI}, 
the forms for both protocols are shown, where the corresponding parameters are defined.

4) **Ligand-based filtering**: on the ligands side, a first filtering step is used by passing 2 ligand-based filter protocols 
to our active and decoy molecules. These are the PAINS and ADME filters, which we described in the previous section. 
The parameters, shown in Fig. \ref{fig:formLFilt}, define the specific rules to follow in ADME and whether to use a 
custom or the default RDKit PAINS definition.

5) **Docking**: this steps involves the execution of 3 independent docking programs (AutoDock-GPU, AutoDock Vina and LeDock) 
...
In our case, the forms (in Fig. \ref{fig:formDock}) allow us to define the number of threads and GPUs (only for AutoDock-GPU) 
...
6) **Rescoring**: in order to combine and compare the docking poses generated by each of the software, we need to first evaluate 
...
7) **Filter and consensus**: finally, the rescored poses can be combined, ranked and the consensus protocol can be applied 
to cluster and extract the most promising docking positions. The forms in Fig. \ref{fig:formCons} refer to the filter and 
...
.. figure:: ../../../_static/images/publications/scipion-chem_vds/consensus_results_inha.png
   :alt: consensus results inha
    ...
    Scipion-chem consensus protocol enrichment. The graph shows the percentage of actives (in blue bars) and the total number of 
    molecules (red dots) for each of the subsets generated in the workflow. The subset "All" represents the original set imported 
    from DUD-E, "LigFilter" after the described ligand-based filtering (which slightly improves the enrichment) and then each of 
    the consensus subset generated by applying a best ODDT score ranking filter for the top x% and consensus docking with parameter N2 or N3.
